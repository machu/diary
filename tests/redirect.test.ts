...
const mockEntries = () => [
  ...
  { id: "2004/20040106p01", data: { date: new Date("2004-01-06") } },
  // multi-entry day (20040107)
  { id: "2004/20040107p01", data: { date: new Date("2004-01-07") } },
...
const makeRedirect =
  ...
  (location: string, status = 302) =>
    new Response(null, {
      status,
      headers: { Location: new URL(location, base).toString() },
    });
...
describe("legacy redirect /diary/YYYYMMDD.html", () => {
  ...
  it("redirects to single entry when only one post exists that day", async () => {
    ...
    expect(res.headers.get("Location")).toBe(
      "https://example.test/posts/20040106/p01",
    );
  });
  ...
  it("redirects to date page when multiple posts exist that day", async () => {
    ...
    expect(res.status).toBe(301);
    expect(res.headers.get("Location")).toBe(
      "https://example.test/posts/20040107/",
    );
  });
    ...
    const res = await mod.GET({
      redirect: (location: string, status = 302) =>
        new Response(null, {
          status,
          headers: {
            Location: new URL(location, "https://example.test").toString(),
          },
        }),
    } as any);
    expect(res.status).toBe(301);
